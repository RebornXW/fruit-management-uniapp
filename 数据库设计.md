...
| 字段名       | 类型           | 描述       | 约束                                     |
|--------------|----------------|------------|-----------------------------------------|
| id           | BIGINT         | 品种ID     | 主键, 自增                              |
| category_id  | BIGINT         | 分类ID     | 外键(fruit_categories.id), 非空         |
...
| 字段名        | 类型             | 描述               | 约束                                             |
|---------------|------------------|--------------------|-------------------------------------------------|
| id            | BIGINT           | 水果ID             | 主键, 自增                                      |
| brand         | VARCHAR(50)      | 品牌               | 非空                                            |
| category_id   | BIGINT           | 水果分类ID         | 外键(fruit_categories.id), 非空                |
| variety_id    | BIGINT           | 水果品种ID         | 外键(fruit_varieties.id), 非空                 |
...
| 字段名 | 类型 | 描述 | 约束 |
|--------|------|------|------|
| id | BIGINT | 客户ID | 主键, 自增 |
| name | VARCHAR(100) | 客户名称 | 非空 |
| phone | VARCHAR(20) | 联系电话 | |
| address | VARCHAR(255) | 地址 | |
| total_sales | DECIMAL(12,2) | 总销售额 (冗余) | 默认0, 由触发器/后端维护 |
| paid_amount | DECIMAL(12,2) | 已付款金额 (冗余) | 默认0 |
| unpaid_amount | DECIMAL(12,2) | 未付款金额 (冗余) | 默认0 |
| remark | TEXT | 备注 | |
| created_at | DATETIME | 创建时间 | 非空 |
| updated_at | DATETIME | 更新时间 | 非空 |
| status | TINYINT | 状态(1:启用 0:禁用) | 非空, 默认1 |
...
| user_id | BIGINT | 销售员ID | 外键(users.id) |
| customer_id | BIGINT | 客户ID | 外键(customers.id) |
| fruit_id | BIGINT | 水果ID | 外键(fruits.id), 非空 |
...
| snapshot | JSON | 销售记录快照 | 可选, 存储当时订单详情 |
| remark | TEXT | 备注 | |
| created_at | DATETIME | 创建时间 | 非空 |
| updated_at | DATETIME | 更新时间 | 非空 |
| deleted | TINYINT | 软删除 | 默认0 |
...
| payment_id | BIGINT | 付款记录ID | 外键(payment_records.id), 非空 |
| sales_id | BIGINT | 销售记录ID | 外键(sales_records.id), 非空 |
...
| snapshot | JSON | 库存记录快照 | 可选, 存储当时水果信息 |
| created_at | DATETIME | 创建时间 | 非空 |
| updated_at | DATETIME | 更新时间 | 非空 |
...
| snapshot | JSON | 操作快照 | 可选, 存储操作前后关键信息 |
| operator_id | BIGINT | 操作员ID | 外键(users.id), 非空 |
| operation_datetime | DATETIME | 操作时间 | 非空 |
| remark | TEXT | 备注 | |
| deleted | TINYINT | 软删除 | 默认0 |
...
| fruit_id | BIGINT | 水果ID | 外键(fruits.id) |
| customer_id | BIGINT | 客户ID | 外键(customers.id) |
